...
class OptionPositionService:
    ...
    def close_position(self, order_id: int, exit_reason: str) -> Dict:
        """
        Close an option position (will need to place closing order separately)
        ...
        Returns:
            Dict with closed position info

        Raises:
            ValueError: If parameters are invalid
            RuntimeError: If position not found or close fails
        """
        if not order_id:
            raise ValueError("order_id is REQUIRED")
        if not exit_reason:
        ...
        logger.info(f"Closing position {order_id}: {exit_reason}")
        ...
        try:
            ...
            # Update P&L one last time
            pnl_info = self.update_position_pnl(order_id)

            # Close position in database
            self.db_manager.close_position(
                order_id=order_id,
                exit_value=pnl_info['current_value'],
                exit_reason=exit_reason,
                realized_pnl=pnl_info['unrealized_pnl']
            )

            logger.info(f"Position {order_id} closed: {exit_reason}, P&L ${pnl_info['unrealized_pnl']:.2f}")
